...
// GET: All tasks already approved by coordinator
// Example: GET http://localhost:7777/coordinator/approved-tasks
router.get("/approved-tasks", (req, res) => {
  const sql = `
    SELECT 
      m.mark_id,
  ...
  pool.query(sql, (error, result) => {
    ...
    if (result.length === 0) {
      return res.status(404).json(errorResponse("No Approved tasks found"));
    }
